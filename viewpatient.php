...
="DELETE FROM patient WHERE patientid='$_GET[delid]'";
	$qsql=mysqli_query($con,$sql);
	if

(mysqli_affected_rows($con) == 1)
	{
		echo "<script>alert('patient record 
...
dataTable">
      ...
      <thead>
        <tr>
          <th width="15%" height="36"><div 

align="center">Name</div></th>
...
width="20%"><div align="center">Patient Profile</div></th>
          <th width="17%"><div 
...
align="center">Action</div></th>
      ...
      <tbody>
       <?php
       $sql ="SELECT *, appointmentid FROM patient,appointment where patient.patientid=appointment.patientid";
       $qsql = 
mysqli_query($con,$sql);
       while($rs = mysqli_fetch_array($qsql))
       {
        ...
        <td><strong>Date</strong>: &nbsp;$rs[admissiondate]<br>
        <strong>Time</strong>: &nbsp;$rs[admissiontime]</td>
        <td>$rs[address]<br>$rs[city] - &nbsp;$rs[pincode]<br>
        Mob No. - $rs[mobileno]</td>
        <td><strong>Blood group</strong> - $rs[bloodgroup]<br><strong>Gender</strong> - &nbsp;$rs[gender]<br>
